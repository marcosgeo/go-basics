...
func getRequests() []repositories.CreateRepoRequest {
	...
	file, err := os.Open("/home/antonio/aluno/go-basics/concurrency-limit/request.txt")
	if err != nil {
		panic(err)
	}
	defer file.Close()
...
func main() {
	...
	buffer := make(chan bool, 10)  // used to limit concurrency to 10 simultaneous requests
	var wg sync.WaitGroup

	go handleResults(&wg, input)

	for _, request := range requests {
		buffer <- true  // get a slot from buffer limiting concurrency
		wg.Add(1)
	...
	// here we can notify the interested by e-mail or other way
}
